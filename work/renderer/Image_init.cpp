...
static void R_CreateCubemapFBO( idImage *image ) {
	image->GenerateFrameBufferCubeImage( 1024, 1024 );
}
...
void idImage::StartBackgroundImageLoad() {
	...
	if ( !bgl.f ) {
		...
		bgl.f = fileSystem->OpenFileRead( "_default.tga" );
	}
	bgl.file.position = 0;
...
// jmarshall - deferred forward rendering stuff
	...
	ImageFromFunction("_currentRenderTargets/xyz/mv1", R_FrameBufferColorTargetImage );
	ImageFromFunction("_currentRenderTargets/xyz/mv2", R_FrameBufferColorTargetImage );
	ImageFromFunction("_currentRenderTargets/xyz/mv3", R_FrameBufferColorTargetImage );
	ImageFromFunction("_currentRenderTargets/xyz/mv4", R_FrameBufferColorTargetImage );
...
===============
*/
idImage *idImageManager::CreateCubemapFBOImage( const char *name ) {
	return ImageFromFunction( name, R_CreateCubemapFBO );
}
...
===============
*/
idImage *idImageManager::CreateDepthFBOImage( const char *name ) {
	return ImageFromFunction( name, R_CreateDepthFBO );
}
