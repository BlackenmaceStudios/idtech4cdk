...
============
*/
__declspec(dllexport) gameExport_t * GetGameAPI( gameImport_t *import ) {
...
void idGameLocal::Init( void ) {
	...
	common->PrintLoadingMessage( "REGISTERING GAME DECLS..." );

	// register game specific decl types
	declManager->RegisterDeclType( "model",				DECL_MODELDEF,		idDeclAllocator<idDeclModelDef> );
	declManager->RegisterDeclType( "export",			DECL_MODELEXPORT,	idDeclAllocator<idDecl> );
	declManager->RegisterDeclType( "objective",			DECL_OBJECTIVE,		idDeclAllocator<bmObjectiveDecl> );
...
// jmarshall
	defferedMaterial = declManager->FindMaterial( "renderer/predeferred_interaction" );
	if(!defferedMaterial) {
		common->FatalError( "Failed to load predeffered material\n");
	}

	postMaterial = declManager->FindMaterial( "renderer/post" );
	if(!postMaterial) {
		common->FatalError( "Failed to load post material\n");
	}

	LoadMainMenuWorld( "maps/game/mainmenu_world" );

	common->PrintLoadingMessage( "COMPILING GAME SCRIPTS" );
// jmarshall end
...
/*
===============
idGameLocal::DrawDefferedPass
===============
*/
void idGameLocal::DrawDefferedPass( int width, int height ) {
	renderSystem->SetColor4( 1, 1, 1, 1.0 );

	renderSystem->DrawStretchPic( 0, 0, width, height, 0, 0, 1, -1, defferedMaterial );
	//renderSystem->CropRenderSize( SCREEN_WIDTH, SCREEN_HEIGHT );
	renderSystem->DrawStretchPic( 0, 0, width, height, 0, 0, 1, -1, postMaterial );

	if(testLightMaterial)
	{
		//renderSystem->DrawStretchPic( 0, 0, 512, 512, 0, 0, 1,1, testLightMaterial );
	}
}
...
/*
=================
idGameLocal::SpawnClientEntity
=================
*/
bmClientEntity * idGameLocal::SpawnClientEntity( idDict &spawnArgs, idRenderEntity *renderEntity, idRenderWorld *world ) {
	int clientEntityNum = 0;

	// If the entity were trying to spawn doesn't have clientSpawnClass defined than its not a valid client entity.
	idStr clientSpawnClass = spawnArgs.GetString( "clientSpawnClass" );
	if(clientSpawnClass.Length() <= 0) {
		return NULL;
	}
	
	// Find the next aval client entity slot.
	for(clientEntityNum  = 0; clientEntityNum  <= MAX_GENTITIES; clientEntityNum ++) {
		if(clientEntityNum  == MAX_GENTITIES) {
			common->Error( "SpawnClientEntity: No more slots\n");
			return NULL;
		}

		if(clientEntities[clientEntityNum] == NULL) {
			break;
		}
	}

	if(!clientSpawnClass.Cmp( "bmClientEntityLight" )) {
		clientEntities[clientEntityNum] = new bmClientEntityLight();
	}
	else if(!clientSpawnClass.Cmp( "bmClientEntityEffects" )) {
		clientEntities[clientEntityNum] = new bmClientEntityEffects();
	}
	else if(!clientSpawnClass.Cmp( "bmClientEntity" )) {
		clientEntities[clientEntityNum] = new bmClientEntity();
	}
	else {
		common->Error("SpawnClientEntity: Unknown client entity spawn type %s\n", clientSpawnClass.c_str() );
	}
	clientEntities[clientEntityNum]->Spawn(spawnArgs, renderEntity, world);

	return clientEntities[clientEntityNum];
}
...
=================
*/
void idGameLocal::SpawnClientEntity( idDict &spawnArgs, idRenderWorld *world ) {

	world->AddClientEntity( spawnArgs );
}

/*
=================
idGameLocal::UpdateClientEntity
=================
*/
void idGameLocal::UpdateClientEntity( bmClientEntity *entity ) {
	entity->Update();
}

/*
=================
idGameLocal::FreeClientEntity
=================
*/
void idGameLocal::FreeClientEntity( bmClientEntity *entity ) {
	entity->Destroy();

	for(int i = 0; i < MAX_GENTITIES; i++) {
		if(entity == clientEntities[i]) {
	...
	common->Error( "FreeClientEntity: Failed to find client entity to destroy\n" );
}
/*
=================
idGameLocal::DrawDebugGui
=================
*/
void idGameLocal::DrawDebugGui( void ) {
	// Load in the debug gui, if it isn't loaded already.
	if(debugGui == NULL)
	{
		debugGui = uiManager->FindGui( "guis/debug.gui", true, false, true );
		if(debugGui == NULL) {
			common->Warning("Failed to load debug user interface.\n");
			return;
		}
	}

	debugGui->Redraw( gameLocal.realClientTime );
}
