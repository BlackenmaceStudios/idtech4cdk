...
void idInterpreter::CallEvent( const function_t *func, int argsize ) {
	...
	int 				pos;
	int 				start;
	INT_PTR				data[ D_EVENT_MAXARGS ]; // jmarshall - INT_PTR
	...
	var.intPtr = ( INT_PTR * )&localstack[ start ];
	...
	for( j = 0, i = 0, pos = type_object.Size(); ( pos < argsize ) || ( format[ i ] != 0 ); i++ ) {
		switch( format[ i ] ) {
		case D_EVENT_INTEGER :
			var.intPtr = ( INT_PTR * )&localstack[ start + pos ];
...
void idInterpreter::CallSysEvent( const function_t *func, int argsize ) {
	...
	for( j = 0, i = 0, pos = 0; ( pos < argsize ) || ( format[ i ] != 0 ); i++ ) {
		switch( format[ i ] ) {
		case D_EVENT_INTEGER :
			source.intPtr = ( INT_PTR * )&localstack[ start + pos ];
			*( INT_PTR * )&data[ i ] = int( *source.floatPtr );
			break;
