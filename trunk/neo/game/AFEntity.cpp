...
===============================================================================
  ...
  bmVehicle_BaseFourWheels

===============================================================================
*/

CLASS_DECLARATION( bmVehicle_Base, bmVehicle_BaseFourWheels )
END_CLASS


/*
================
bmVehicle_BaseFourWheels::bmVehicle_BaseFourWheels
================
*/
bmVehicle_BaseFourWheels::bmVehicle_BaseFourWheels( void ) {
	int i;
...
/*
================
bmVehicle_BaseFourWheels::Spawn
================
*/
void bmVehicle_BaseFourWheels::Spawn( void ) {
	int i;
	...
	for ( i = 0; i < 4; i++ ) {
		...
		if ( !wheelBodyName[0] ) {
			gameLocal.Error( "bmVehicle_BaseFourWheels '%s' no '%s' specified", name.c_str(), wheelBodyKeys[i] );
		}
		wheels[i] = af.GetPhysics()->GetBody( wheelBodyName );
		if ( !wheels[i] ) {
			gameLocal.Error( "bmVehicle_BaseFourWheels '%s' can't find wheel body '%s'", name.c_str(), wheelBodyName );
		}
		wheelJointName = spawnArgs.GetString( wheelJointKeys[i], "" );
		if ( !wheelJointName[0] ) {
			gameLocal.Error( "bmVehicle_BaseFourWheels '%s' no '%s' specified", name.c_str(), wheelJointKeys[i] );
		}
		wheelJoints[i] = animator.GetJointHandle( wheelJointName );
		if ( wheelJoints[i] == INVALID_JOINT ) {
			gameLocal.Error( "bmVehicle_BaseFourWheels '%s' can't find wheel joint '%s'", name.c_str(), wheelJointName );
		}
	}
	...
	for ( i = 0; i < 2; i++ ) {
		...
		if ( !steeringHingeName[0] ) {
			gameLocal.Error( "bmVehicle_BaseFourWheels '%s' no '%s' specified", name.c_str(), steeringHingeKeys[i] );
		}
		steering[i] = static_cast<idAFConstraint_Hinge *>(af.GetPhysics()->GetConstraint( steeringHingeName ));
		if ( !steering[i] ) {
			gameLocal.Error( "bmVehicle_BaseFourWheels '%s': can't find steering hinge '%s'", name.c_str(), steeringHingeName );
		}
	}
...
/*
================
bmVehicle_BaseFourWheels::Think
================
*/
void bmVehicle_BaseFourWheels::Think( void ) {
	int i;
...
===============================================================================
  ...
  bmVehicle_BaseSixWheels

===============================================================================
*/

CLASS_DECLARATION( bmVehicle_Base, bmVehicle_BaseSixWheels )
END_CLASS

	/*
================
bmVehicle_BaseSixWheels::bmVehicle_BaseSixWheels
================
*/
bmVehicle_BaseSixWheels::bmVehicle_BaseSixWheels( void ) {
	int i;
...
/*
================
bmVehicle_BaseSixWheels::Spawn
================
*/
void bmVehicle_BaseSixWheels::Spawn( void ) {
	int i;
	static const char *wheelBodyKeys[] = {
		"wheelBodyFrontLeft",
		"wheelBodyFrontRight",
	...
	for ( i = 0; i < 6; i++ ) {
		wheelBodyName = spawnArgs.GetString( wheelBodyKeys[i], "" );
		if ( !wheelBodyName[0] ) {
			gameLocal.Error( "bmVehicle_BaseSixWheels '%s' no '%s' specified", name.c_str(), wheelBodyKeys[i] );
		}
		wheels[i] = af.GetPhysics()->GetBody( wheelBodyName );
		if ( !wheels[i] ) {
			gameLocal.Error( "bmVehicle_BaseSixWheels '%s' can't find wheel body '%s'", name.c_str(), wheelBodyName );
		}
		wheelJointName = spawnArgs.GetString( wheelJointKeys[i], "" );
		if ( !wheelJointName[0] ) {
			gameLocal.Error( "bmVehicle_BaseSixWheels '%s' no '%s' specified", name.c_str(), wheelJointKeys[i] );
		}
		wheelJoints[i] = animator.GetJointHandle( wheelJointName );
		if ( wheelJoints[i] == INVALID_JOINT ) {
			gameLocal.Error( "bmVehicle_BaseSixWheels '%s' can't find wheel joint '%s'", name.c_str(), wheelJointName );
		}
	}
	...
	for ( i = 0; i < 4; i++ ) {
		steeringHingeName = spawnArgs.GetString( steeringHingeKeys[i], "" );
		if ( !steeringHingeName[0] ) {
			gameLocal.Error( "bmVehicle_BaseSixWheels '%s' no '%s' specified", name.c_str(), steeringHingeKeys[i] );
		}
		steering[i] = static_cast<idAFConstraint_Hinge *>(af.GetPhysics()->GetConstraint( steeringHingeName ));
		if ( !steering[i] ) {
			gameLocal.Error( "bmVehicle_BaseSixWheels '%s': can't find steering hinge '%s'", name.c_str(), steeringHingeName );
		}
	}
...
/*
================
bmVehicle_BaseSixWheels::Think
================
*/
void bmVehicle_BaseSixWheels::Think( void ) {
	int i;
	float force = 0.0f, velocity = 0.0f, steerAngle = 0.0f;
	idVec3 origin;
	idMat3 axis;
