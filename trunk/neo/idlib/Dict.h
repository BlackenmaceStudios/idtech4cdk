...
// jmarshall -- this is used in the managed code.
struct idKeyValueInstance {
	const char			*key;
	const char			*value;
};
// jmarshall end
...
// jmarshall
	idKeyValueInstance  GetKeyValInstance( int index );
// jmarshall end
...
// jmarshall
ID_INLINE idKeyValueInstance idDict::GetKeyValInstance( int index )
{
	const idKeyValue *val = GetKeyVal( index );
	idKeyValueInstance instance;

	instance.key = val->GetKey().c_str();
	instance.value = val->GetValue().c_str();
