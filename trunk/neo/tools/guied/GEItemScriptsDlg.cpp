...
{
	WNDPROC wndproc = (WNDPROC) GetWindowLongPtr ( hwnd, GWL_USERDATA );
...
bool GEItescriptsDlg_Init ( HWND hwnd )
{
	...
	window = (idWindow*)GetWindowLongPtr ( hwnd, GWL_USERDATA );
	assert ( window );
	...
	SetWindowLongPtr ( script, GWL_USERDATA, GetWindowLongPtr ( script, GWL_WNDPROC ) );
	SetWindowLongPtr ( script, GWL_WNDPROC, (LONG_PTR) GEScriptEdit_WndProc );					
		...
		{		
			...
			*/
			...
			else
			{
				...
				{
// jmarshall
					if ( idStr::Icmp ( ((idUserInterfaceManagerLocal *)uiManager)->ScriptNames()[i], token ) == 0 )
					{
						scriptName = ((idUserInterfaceManagerLocal *)uiManager)->ScriptNames()[i];
						break;
					}
// jmarshall end
				}								
