...
void rvGEWorkspace::Render ( HDC hdc )
{
	...
	viewDef->renderView.x = mRect.x;
	viewDef->renderView.y = mWindowHeight - mRect.y - mRect.h;
	viewDef->renderView.width = mRect.w;
	viewDef->renderView.height = mRect.h;
	viewDef->scissor.x1 = 0;
	viewDef->scissor.y1 = 0;
	viewDef->scissor.x2 = mRect.w;
	viewDef->scissor.y2 = mRect.h;
...
int	rvGEWorkspace::HandleMouseMove ( WPARAM wParam, LPARAM lParam )
{
	...
	if ( mDragType == rvGESelectionMgr::HT_MOVE && sys->Milliseconds() - mDragTime <= 50 )
	{
		return 0;
	}
...
{
// jmarshall - moved this over to uilocal
	idWindow*			window = mInterface->AllocWindow( type );
// jmarshall end
