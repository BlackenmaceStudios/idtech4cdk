...
BOOL CXYWnd::PreCreateWindow(CREATESTRUCT &cs) {
	...
	return idManagedEditorWindow::PreCreateWindow(cs);
}
#endif
...
{
	if ( idManagedEditorWindow::OnCmdMsg( nID, nCode, pExtra, pHandlerInfo ) ) {
		return TRUE;
	}
...
void CXYWnd::OnAddEntityEvent( const char *entityType, CPoint pt ) {
	CreateRightClickEntity(this, pt.x, pt.y, (char *)entityType);
}
...
void CXYWnd::XY_Draw() {
	...
	renderDevice->SetDeviceState( GLS_DEFAULT );
...
void CXYWnd::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) {
	...
	// idManagedEditorWindow::OnKeyUp(nChar, nRepCnt, nFlags);
}
...
void CXYWnd::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS FAR *lpncsp) {
	idManagedEditorWindow::OnNcCalcSize(bCalcValidRects, lpncsp);
}
...
void CXYWnd::OnKillFocus(CWnd *pNewWnd) {
	idManagedEditorWindow::OnKillFocus(pNewWnd);
//	SendMessage(WM_NCACTIVATE, FALSE, 0); // jmarshall
}
...
void CXYWnd::OnSetFocus(CWnd *pOldWnd) {
	idManagedEditorWindow::OnSetFocus(pOldWnd);
//	SendMessage(WM_NCACTIVATE, TRUE, 0); // jmarshall end
}
...
BOOL CXYWnd::OnEraseBkgnd(CDC *pDC) {
	...
	// return idManagedEditorWindow::OnEraseBkgnd(pDC);
}
