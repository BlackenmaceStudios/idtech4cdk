...
// jmarshall
		...
		renderDevice->RenderSurfaceFromCache( surf->geometry );
/*
...
void Brush_Update( void ) {
	if(selectedEntity != NULL) {
		if ( selectedEntity->epairs.GetString( "model" ) != NULL) {
			idBounds	bo;
			idVec3	mins, maxs;

			selected_brushes.next->modelHandle = renderModelManager->FindModel( selectedEntity->epairs.GetString( "model" ) );
			...
			VectorAdd(mins, selectedEntity->origin, mins);
			VectorAdd(maxs, selectedEntity->origin, maxs);
...
void DrawProjectedLight(brush_t *b, bool bSelected, bool texture) {
	...
	renderSystem->RenderLightFrustum( parms, planes );

	tri = renderTriSurfaceManager->PolytopeSurface(6, planes, NULL);
	...
	renderTriSurfaceManager->FreeStaticTriSurf(tri);
