...
// Sys_OpenFileDialog
//
const char * Sys_OpenFileDialog( const char *Title, const char *fileExt, const char *defaultExt ) {
	static char path[512];

	CFileDialog fOpenDlg(TRUE, defaultExt, "", OFN_HIDEREADONLY|OFN_FILEMUSTEXIST, fileExt, g_pParentWnd);
  
	fOpenDlg.m_pOFN->lpstrTitle=Title;
  
	fOpenDlg.m_pOFN->lpstrInitialDir=fileSystem->RelativePathToOSPath( "models", "fs_basepath" );

	if(fOpenDlg.DoModal()==IDOK) {
		strcpy( path, fOpenDlg.GetPathName() );
		return &path[ 0 ];
	}
...
// jmarshall
	...
	ON_COMMAND(ID_TOOLS_SOUNDEDITOR, OnOpenSoundEditor)
	ON_COMMAND(ID_TOOLS_DECLEDITOR, OnOpenDeclEditor)
	ON_COMMAND(ID_TOOLS_MATERIALEDITOR, OnOpenMaterialEditor)
	
	ON_COMMAND(ID_TOOLS_SOUNDINGAMEEDITOR, OnOpenSoundEditorInGame)
	ON_COMMAND(ID_TOOLS_PARTICLEINGAMEEDITOR, OnOpenParticleEditor)
	ON_COMMAND(ID_TOOLS_AFINGAMEEDITOR, OnOpenAFEditor)

	ON_COMMAND(ID_TOOLS_TESTMAP, OnTestMap)
	ON_COMMAND(ID_TOOLS_DEVTESTMAP, OnDevTestMap)
	
	ON_COMMAND(ID_TOOLS_BUILDANDTESTMAP, OnBuildAndTestMap)
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct) {
	...
	// Set the editor icon -- HACK hardcoded editor icon.
	...
	// jmarshall -- drop down isn't used, add this stuff to the side bar.
	eclass_t	*e;
	CString		strActive;
	CString		strLast;
	CString		strName;
	for (e = eclass; e; e = e->next) {
		...
		if (n_ > 0) {
			_bstr_t strLeft((LPCTSTR)strName.Left(n_));
			_bstr_t strRight((LPCTSTR)strName.Right(strName.GetLength() - n_ - 1));

			toolsManaged->AddNewEntityType(strLeft, strRight);
		}
...
BOOL CMainFrame::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext *pContext) {
	...
	toolsManaged->CreateEditorWindows( (__int64)GetSafeHwnd() );
	...
	m_pXYWnd = new CXYWnd();
	m_pXYWnd->Create("Viewport", "", QE3_CHILDSTYLE, rect, this, 1235);
	m_pXYWnd->SetViewType(XY);

	m_pXZWnd = new CXYWnd();
	m_pXZWnd->Create("Viewport", "", QE3_CHILDSTYLE, rect, this, 1236);
	m_pXZWnd->SetViewType(XZ);

	m_pYZWnd = new CXYWnd();
	m_pYZWnd->Create("Viewport", "", QE3_CHILDSTYLE, rect, this, 1237);
...
void RunBsp (const char *command) {
	...
	if (idStr::Icmpn(command, "bspext", strlen("runbsp")) == 0) {
		...
		cmdSystem->BufferCommandText( CMD_EXEC_NOW, sys );
...
void CMainFrame::OnActivate(UINT nState, CWnd *pWndOther, BOOL bMinimized) {
	...
	if ( nState != WA_INACTIVE ) {
		...
		if (::IsWindowVisible(win32.hWnd)) {
			if(common->GetActiveEditors() & EDITOR_SOUND) {
				SoundEditorShutdown();
			}

			if(common->GetActiveEditors() & EDITOR_AF) {
				AFEditorShutdown();
			}

			if(common->GetActiveEditors() & EDITOR_PARTICLE) {
...
bool CMainFrame::VerifyMapBuildable(const char *opname, bool checkProc) {
	// FixMe: This function needs to do more stuff...
	if(strlen(currentmap) <= 0 || !strcmp(currentmap, "unnamed.map")) {
		MessageBox( va( "Before you can %s you need to save your map", opname ) );
		return false;
	}
	if (!active_brushes.next) {
		MessageBox( va( "Before you can %s you need geometry in your map", opname ) );
		return false;
	}

	// This is how not to join strings, looks like a clusterf#
	if(checkProc) {
		idStr relativepath = fileSystem->OSPathToRelativePath(va("%s", currentmap) );
		...
		if(f == NULL) {
			...
			cmdSystem->BufferCommandText( CMD_EXEC_NOW, va("dmap %s", relativepath.c_str() + strlen("maps\\")) );
		}
		else
		{
...
void CMainFrame::OnOpenGUIEditor() {
	// If the editor is open, don't open it again.
	if(common->GetActiveEditors() & EDITOR_GUI) {
		return;
	}
	GUIEditorInit();
}

void CMainFrame::OnOpenScriptEditor() {
	// If the editor is open, don't open it again.
	if(common->GetActiveEditors() & EDITOR_SCRIPT) {
		return;
	}
	ScriptEditorInit(NULL);
}
void CMainFrame::OnOpenSoundEditor() {
	// If the editor is open, don't open it again.
	if(common->GetActiveEditors() & EDITOR_SOUND) {
		return;
	}
...
void CMainFrame::OnOpenDeclEditor() {
	// If the editor is open, don't open it again.
	if(common->GetActiveEditors() & EDITOR_DECL) {
		return;
	}
	DeclBrowserInit(NULL);
}
void CMainFrame::OnOpenMaterialEditor() {
	// If the editor is open, don't open it again.
	if(common->GetActiveEditors() & EDITOR_MATERIAL) {
		return;
	}
	MaterialEditorInit();
}

void CMainFrame::OnOpenModelStudio() {
	// If the editor is open, don't open it again.
	if(common->GetActiveEditors() & EDITOR_MODELSTUDIO) {
		return;
	}
...
void CMainFrame::OnOpenParticleEditor() {
	...
	if(!VerifyMapBuildable("Open a ingame editor")) {
		return;
	}

	const char *relativepath = fileSystem->OSPathToRelativePath(va("%s", currentmap) );
	cmdSystem->BufferCommandText( CMD_EXEC_NOW, va("devmap %s\n", relativepath + strlen("maps\\")) );
	cmdSystem->BufferCommandText( CMD_EXEC_NOW, "showGameWindow\n" );
	cmdSystem->BufferCommandText( CMD_EXEC_NOW, "editParticles\n" );
}
void CMainFrame::OnOpenAFEditor() {
	// If the editor is open, don't open it again.
	if(common->GetActiveEditors() & EDITOR_AF) {
		return;
	}
	...
	cmdSystem->BufferCommandText( CMD_EXEC_NOW, va("devmap %s\n", relativepath + strlen("maps\\")) );
	cmdSystem->BufferCommandText( CMD_EXEC_NOW, "showGameWindow\n" );
	cmdSystem->BufferCommandText( CMD_EXEC_NOW, "editAFs\n" );
}

void CMainFrame::OnOpenSoundEditorInGame() {
	// If the editor is open, don't open it again.
	if(common->GetActiveEditors() & EDITOR_SOUND) {
		return;
	}
	...
	cmdSystem->BufferCommandText( CMD_EXEC_NOW, va("devmap %s\n", relativepath + strlen("maps\\")) );
	cmdSystem->BufferCommandText( CMD_EXEC_NOW, "showGameWindow\n" );
	cmdSystem->BufferCommandText( CMD_EXEC_NOW, "editSounds\n" );
}

void CMainFrame::OnTestMap() {
	if(!VerifyMapBuildable("Test your map", true)) {
		return;
	}

	const char *relativepath = fileSystem->OSPathToRelativePath(va("%s", currentmap) );
	cmdSystem->BufferCommandText( CMD_EXEC_NOW, "showGameWindow\n" );
	cmdSystem->BufferCommandText( CMD_EXEC_NOW, va("map %s\n", relativepath + strlen("maps\\")) );
	
}
void CMainFrame::OnDevTestMap() {
	if(!VerifyMapBuildable("Developer Test your map", true)) {
		return;
	}
...
void CMainFrame::OnBuildAndTestMap() {
	if(!VerifyMapBuildable("Build and Developer Test your map")) {
		return;
	}

	const char *relativepath = fileSystem->OSPathToRelativePath(va("%s", currentmap) );

	cmdSystem->BufferCommandText( CMD_EXEC_NOW, va("dmap %s\n", relativepath) );
		cmdSystem->BufferCommandText( CMD_EXEC_NOW, "showGameWindow\n" );
	cmdSystem->BufferCommandText( CMD_EXEC_NOW, va("devmap %s\n", relativepath + strlen("maps\\") ));
