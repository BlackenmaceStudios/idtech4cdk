...
{
	rvGEMaskEdit* edit = (rvGEMaskEdit*)GetWindowLongPtr ( hWnd, GWL_USERDATA );
...
void MaskEdit_Attach ( HWND hWnd, const char* invalid )
{
	...
	edit->mProc = (WNDPROC)GetWindowLongPtr ( hWnd, GWL_WNDPROC );
	strcpy ( edit->mInvalid, invalid );
	SetWindowLongPtr ( hWnd, GWL_USERDATA, (LONG_PTR)edit );
	SetWindowLongPtr ( hWnd, GWL_WNDPROC, (LONG_PTR)MaskEdit_WndProc );
}
