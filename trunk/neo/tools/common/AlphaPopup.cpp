...
LRESULT CALLBACK AlphaSlider_WndProc ( HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
	switch ( msg )
	{
		case WM_LBUTTONDOWN:
		{
			...
			if ( v < 0 ) v = 0;
			if ( v > 1.0f ) v = 1.0f;
			SetWindowLongPtr ( hwnd, GWL_USERDATA, MAKELONG(0x8000,(unsigned short)(255.0f * v)) );
		...
		case WM_MOUSEMOVE:		
			if ( LOWORD(GetWindowLongPtr ( hwnd, GWL_USERDATA ) ) & 0x8000 )
			{
				RECT  rClient;
				float v;
		...
		case WM_LBUTTONUP:		
			if ( LOWORD(GetWindowLongPtr ( hwnd, GWL_USERDATA ) ) & 0x8000 )
			{
				...
				InvalidateRect ( hwnd, NULL, FALSE );
				ReleaseCapture ( );
				SendMessage ( GetParent ( hwnd ), WM_COMMAND, MAKELONG(GetWindowLongPtr (hwnd,GWL_ID),0), 0 );
			}
			break;
		...
		case WM_PAINT:
		{
			...
			// Draw the thumb
			RECT rThumb;
			short s = HIWORD(GetWindowLongPtr ( hwnd, GWL_USERDATA ));
