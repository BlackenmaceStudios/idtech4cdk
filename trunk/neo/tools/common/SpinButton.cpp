...
{
	SetWindowLongPtr ( hWnd, GWL_USERDATA, (LONG_PTR)(inc * 100.0f) );
}

void SpinButton_SetRange ( HWND hWnd, float minRange, float maxRange )
{
	SendMessage ( hWnd, UDM_SETRANGE32, (LONG_PTR)(minRange*100.0f), (LONG_PTR)(maxRange*100.0f) );	
}
...
void SpinButton_HandleNotify ( NMHDR* hdr )
{
	...
	float inc = (float)GetWindowLongPtr ( hdr->hwndFrom, GWL_USERDATA );
	if ( inc == 0 )
	{
		inc = 100.0f;
		SetWindowLongPtr ( hdr->hwndFrom, GWL_USERDATA, 100 );
	}
	inc /= 100.0f;
	...
	{
		float minRangef = (float)(LONG_PTR)minRange / 100.0f;
