/**
 * $Id: edgehash.c,v 1.4 2005/08/23 02:29:22 zuster Exp $
 *
...
EdgeHash *BLI_edgehash_new(void) {
	EdgeHash *eh= (EdgeHash*)MEM_mallocN(sizeof(*eh), "EdgeHash");				// (EdgeHash*) added by Swanny 19/05/05
	eh->cursize= 0;
	eh->nentries= 0;
	eh->nbuckets= hashsizes[eh->cursize];
	
	eh->buckets= (Entry**)malloc(eh->nbuckets*sizeof(*eh->buckets));			// (Entry**) added by Swanny 19/05/05
...
void BLI_edgehash_insert(EdgeHash *eh, int v0, int v1, void *val) {
	unsigned int hash;
	Entry *e= (Entry*)malloc(sizeof(*e));										// (Entry*) added by Swanny 19/05/05
...
EdgeHashIterator *BLI_edgehashIterator_new(EdgeHash *eh) {
	EdgeHashIterator *ehi= (EdgeHashIterator*)malloc(sizeof(*ehi));				// (EdgeHashIterator*) added by Swanny 19/05/05
	ehi->eh= eh;
	ehi->curEntry= NULL;
	ehi->curBucket= -1;
	while (!ehi->curEntry) {
