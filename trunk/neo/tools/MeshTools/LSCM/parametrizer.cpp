...
static bool p_intersect_line_2d_dir(float *v1, float *dir1, float *v2, float *dir2, float *isect)
{
	float lmbda, div;
...
#if 0
static bool p_intersect_line_2d(float *v1, float *v2, float *v3, float *v4, float *isect)
{
	float dir1[2], dir2[2];
...
static bool p_face_exists(PHandle *handle, PHashKey *vkeys, int i1, int i2, int i3)
{
...
static bool p_edge_has_pair(PHandle *handle, PEdge *e, PEdge **pair, bool impl)
{
	PHashKey key;
	PEdge *pe;
	PVert *v1, *v2;
	...
	*pair = NULL;
	...
	return (bool)(*pair != NULL);
}

static bool p_edge_connect_pair(PHandle *handle, PEdge *e, PEdge ***stack, bool impl)
{
	PEdge *pair = NULL;
	...
	return ((bool)(e->pair != NULL));
}

static int p_connect_pairs(PHandle *handle, bool impl)
{
...
{
	PChart **charts = (PChart**)MEM_mallocN(sizeof*charts * ncharts, "PCharts"), *nchart;
	PFace *f, *nextf;
	int i;
...
static PFace *p_face_add_construct(PHandle *handle, ParamKey key, ParamKey *vkeys,
                                   ...
                                   bool *pin, bool *select)						// ParamBool
{
...
static bool p_quad_split_direction(PHandle *handle, float **co, PHashKey *vkeys)
{
	float fac= VecLenf(co[0], co[2]) - VecLenf(co[1], co[3]);
	bool dir = (bool)(fac <= 0.0f);
...
//	void* J2dt = MEM_mallocN(sizeof(float)*sys->nangles*3, "ABFj2dt");
//	sys->J2dt =  MEM_mallocN(sizeof(float)*sys->nangles*3, "ABFj2dt");
	sys->J2dt =  (float**)MEM_mallocN(sizeof(float)*sys->nangles*3, "ABFj2dt");

	for(int Index =0; Index < sys->nangles; Index++)
	{
		sys->J2dt[Index] = (float*)MEM_mallocN(sizeof(float)*3, "ABFj2dt");
	}
	...
	sys->minangle = 7.5f*(float)M_PI/180.0f;
...
static float p_abf_compute_gradient(PAbfSystem *sys, PChart *chart)
{
	...
	for (v=chart->verts; v; v=v->nextlink) {
		if (v->flag & PVERT_INTERIOR) {
			float gplanar = -2*(float)M_PI, glength;
...
static bool p_chart_abf_solve(PChart *chart)
{
	...
	for (v=chart->verts; v; v=v->nextlink) {
		if (v->flag & PVERT_INTERIOR) {
			...
			scale = (anglesum == 0.0f)? 0.0f: 2*(float)M_PI/anglesum;
...
static void p_chart_extrema_verts(PChart *chart, PVert **pin1, PVert **pin2)
{
	...
	minv[0] = minv[1] = minv[2] = (float)1e20;
	maxv[0] = maxv[1] = maxv[2] = (float)-1e20;
...
{
	PVert *v, *pin1, *pin2;
	bool select = P_FALSE, deselect = P_FALSE;
	int npins = 0, id = 0;
...
//			if (v->flag & PVERT_SELECT)
	...
	if ((live && (!select || !deselect)) || (npins == 1)) 
	{
		...
		if (abf == true)				//  && live
		{
			bool p_chart_abf_solve(PChart *chart);
...
//****************************************************************************************************************************************************************
// start of Swanny Bodgery

// Put back in the old LSCM solution from Blender 2.41


static void p_old_style_pinning(PChart *chart, PVert** pinMin, PVert** pinMax, float* p1, float* p2)
{
	float umin= (float)1.0e30;
	float umax= (float)-1.0e30;
	int vmin = 0;
	int vmax = 2;
	float uv[2];
...
static void p_chart_min_max_cent_rad(PChart *chart, float *min, float *max, float *center, float *radius)
{
	int vertcount;					// a, lastvert, 
	float sub[3];					// *co, 
...
//	PVert *v;
	...
	for (f=chart->faces; f; f=f->nextlink) 
	{
		...
		VecAddf(center, center, v1->co);
		VecAddf(center, center, v2->co);
		VecAddf(center, center, v3->co);

		DO_MINMAX(v1->co, min, max);
		DO_MINMAX(v2->co, min, max);
		DO_MINMAX(v3->co, min, max);
...
static bool p_chart_lscm_old_solve(PChart *chart)
{
	float min[3], max[3], center[3], radius, p1[3], p2[3];
	PVert *v, *pin1 = chart->u.lscm.pin1, *pin2 = chart->u.lscm.pin2;
	PFace *f;

	p_chart_min_max_cent_rad(chart, min, max, center, &radius);

	lscm_projection_axes(min, max, p1, p2);

	if (chart->m_CreatePins == true) 
	{
		/* not enough pins, lets find some ourself */
		PVert* pin1;
		PVert* pin2;

		p_old_style_pinning(chart, &pin1, &pin2, p1, p2);
		chart->u.lscm.pin1 = pin1;
		chart->u.lscm.pin2 = pin2;
	}
	...
	for (f=chart->faces; f; f=f->nextlink) 
	{
		...
		* edge length is lost, as this algorithm is angle based */
		lscm_normalize(co1, center, radius);
		lscm_normalize(co2, center, radius);
		lscm_normalize(co3, center, radius);
...
// end of Swanny Bodgery
//****************************************************************************************************************************************************************




static bool p_chart_lscm_new_solve(PChart *chart)
{
	PVert *v, *pin1 = chart->u.lscm.pin1, *pin2 = chart->u.lscm.pin2;
	PFace *f;
	...
	for (f=chart->faces; f; f=f->nextlink) {
		...
		nlBegin(NL_ROW);
		nlCoefficient(2*v1->u.id,   cosine - 1.0f);
		...
		nlCoefficient(2*v1->u.id+1, cosine - 1.0f);
...
static bool p_pack_try(PHandle *handle, float side)
{
	PChart *chart;
	...
	packx= packy= 0.0;
	rowh= 0.0;
	groupw= 1.0f/sqrt((float)handle->ncharts);
...
void p_charts_pack(PHandle *handle)
{
	...
	for (i = 0; i < handle->ncharts; i++) {
		...
		chart->u.pack.rescale *= 1.01f;			// give us a little bit of breathing room
	...
	maxside = (((int)sqrt((float)(handle->ncharts-1))+1))*maxside;

	if (minside < maxside) { /* should always be true */

		for (i = 0; i < PACK_SEARCH_DEPTH; i++) {
			if (p_pack_try(handle, (minside+maxside)*0.5f + (float)1e-5))
...
static bool p_chart_convex_hull(PChart *chart, PVert ***verts, int *nverts, int *right)
{
...
void p_chart_rotate_minimum_area(PChart *chart)
{
	...
	angle += 1.570796327f;								// add on 90 degrees
...
static bool p_triangle_inside(SmoothTriangle *t, float *co)
{
	float b[3];
...
{
	SmoothNode *node = (SmoothNode*)BLI_memarena_alloc(arena, sizeof *node);
	...
	t1 = (SmoothTriangle**)MEM_mallocN(sizeof(SmoothTriangle)*ntri, "PNodeTri1");
	t2 = (SmoothTriangle**)MEM_mallocN(sizeof(SmoothTriangle)*ntri, "PNodeTri1");
...
static bool p_node_intersect(SmoothNode *node, float *co)
{
	int i;
...
{
	PEdge *e;
	float *lengths = (float*)MEM_mallocN(sizeof(chart->edges)*chart->nedges, "PMedianLength");
	float median;
	int i;
...
void p_smooth(PChart *chart)
{
	...
	nodes = (float*)MEM_mallocN(sizeof(float)*nsize, "PSmoothNodes");
	nodesx = (float*)MEM_mallocN(sizeof(float)*nsize, "PSmoothNodesX");
	nodesy = (float*)MEM_mallocN(sizeof(float)*nsize, "PSmoothNodesY");
	oldnodesx = (float*)MEM_mallocN(sizeof(float)*nsize, "PSmoothOldNodesX");
	oldnodesy = (float*)MEM_mallocN(sizeof(float)*nsize, "PSmoothOldNodesY");
	hedges = (float*)MEM_mallocN(sizeof(float)*esize, "PSmoothHEdges");
	vedges = (float*)MEM_mallocN(sizeof(float)*esize, "PSmoothVEdges");
...
//		error("Not enough memory for area smoothing grid.");
	...
	/* create bsp */
	t = triangles = (SmoothTriangle*)MEM_mallocN(sizeof(SmoothTriangle)*esize*2, "PSmoothTris");
	trip = tri = (SmoothTriangle**)MEM_mallocN(sizeof(SmoothTriangle*)*esize*2, "PSmoothTriP");
...
{
	PHandle *handle = (PHandle*)MEM_callocN(sizeof*handle, "PHandle");
...
void param_construct_end(ParamHandle *handle, bool fill, bool impl)		// ParamBool
{
	PHandle *phandle = (PHandle*)handle;
...
void param_lscm_begin(ParamHandle *handle, bool live, bool abf)		// ParamBool
{
	PHandle *phandle = (PHandle*)handle;
	PFace *f;
	int i;
...
//			if(gABF == false)
//			{
//				result = p_chart_lscm_old_solve(chart);
//			}
//			else
			{
				result = p_chart_lscm_new_solve(chart);
			}


			if (result && !(chart->flag & PCHART_NOPACK))			// re-added
