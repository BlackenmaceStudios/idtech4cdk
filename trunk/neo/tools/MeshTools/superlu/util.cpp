...
{
    DNformat* Astore = (DNformat*)A->Store;					// (DNformat*) added by Swanny 19/05/05
...
void super_stats(int nsuper, int *xsup)
{
    ...
    for (i = 0; i <= nsuper; i++) {
        isize = xsup[i+1] - xsup[i];
        whichb = (int)((float)isize / max_sup_size * NBUCKS);			// (int) added by swanny 20/05/05
    ...
    for (i = 0; i < NBUCKS; i++) {
        bl = (int)((float) i * max_sup_size / NBUCKS);					// (int) added by swanny 20/05/05
        bh = (int)((float) (i+1) * max_sup_size / NBUCKS);				// (int) added by swanny 20/05/05
        printf("\tsnode: %d-%d\t\t%d\n", bl+1, bh, bucket[i]);
    }
...
float SpaSize(int n, int np, float sum_npw)
{
    return (sum_npw*8 + np*8 + n*4)/1024.0f;
}

float DenseSize(int n, float sum_nw)
{
    return (sum_nw*8 + n*8)/1024.0f;
}
