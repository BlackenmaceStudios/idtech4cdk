...
// jmarshall - deferred
idCVar r_deferredRenderer( "r_deferredRenderer", "1", CVAR_RENDERER | CVAR_CHEAT | CVAR_INTEGER, "draw the scene using the optimized deferred rendering system",  R_CvarDeferredValueChanged_f );
// jmarshall end
...
=================
*/
bool R_CheckExtension( char *name, bool required ) {
...
vidmode_t r_vidModes[] = {
	{ "Mode  0: 1280x720",		SCREEN_WIDTH, SCREEN_HEIGHT }
};
...
static bool R_GetModeInfo( int *width, int *height, int mode ) {
	...
	if ( mode >= s_numVidModes ) {
		common->Warning("R_Mode >= s_numVidModes, defaulting...\n");
		mode = 0;
	}
...
==================
*/
void GL_CheckErrors( bool error ) {
    int		err;
    char	s[64];
	int		i;
	...
	for ( i = 0 ; i < 10 ; i++ ) {
		...
		if(!error)
		{
			common->Warning( "GL_CheckErrors: %s\n", s );
		}
		else {
			common->FatalError( "GL_CheckErrors: %s\n", s );
		}
		//}
	}
}
...
void R_InitMaterials( void ) {
	...
	tr.postDiffuseMtr = declManager->FindMaterial( "renderer/postdiffuse" );
}
