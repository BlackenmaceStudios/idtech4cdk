...
void idRenderModelStatic::InitFromFile( const char *fileName ) {
	...
	if ( extension.Icmp( MD5_STATIC_EXT ) == 0 ) {
		loaded		= LoadMD5Static( name );
		reloadable	= true;
	} else {
...
/*
=================
idRenderModelStatic::LoadMD5Static
=================
*/
// jmarshall - MD5 static mesh format.
bool idRenderModelStatic::LoadMD5Static( const char *fileName ) {
	float			uOffset, vOffset, textureSin, textureCos;
	float			uTiling, vTiling;
	bmMD5StaticModel model;
	srfTriangles_t *tri;

	if(!MD5Static_LoadModel( fileName, model )) {
		return false;
	}
	...
	for( int i = 0; i < model.NumMeshes(); i++) {
		...
		for ( int j = 0; j < tri->numVerts; j++ ) {
			tri->verts[ j ].Clear();
			tri->verts[ j ].xyz = model.verts[ mesh->startVertex + j ].xyz;
			tri->verts[ j ].normal = model.verts[ mesh->startVertex + j ].normal;
