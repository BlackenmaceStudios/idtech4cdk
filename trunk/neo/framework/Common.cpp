...
public:
	...
	virtual int					GetActiveEditors() { return com_editors; }
	virtual void				KeyClearStates( void ) { idKeyInput::ClearStates(); }
	virtual bool				EditorActive( void ) { return com_editorActive; }
...
void idCommonLocal::VPrintf( const char *fmt, va_list args ) {
	...
	// Echo to the debugger if one is attached.
...
#ifdef ID_ALLOW_TOOLS
	toolManager.InitTool( tool, dict );
#endif
}
...
// jmarshall
	...
	if(!devToolsStartup) {
		loadMsgPrefix = GetLanguageDict()->GetString( "#str_04341" );
	}
	else {
		loadMsgPrefix = GetLanguageDict()->GetString( "#str_04342" );
	}
// jmarshall end
}
...
void idCommonLocal::PrintLoadingMessage( const char *msg ) {
	...
	int len = strlen( msg ) + loadMsgPrefix.Length();
	renderSystem->DrawSmallStringExt( ( SCREEN_WIDTH - len * SMALLCHAR_WIDTH ) / 2, SCREEN_HEIGHT - 70, va("%s %s", loadMsgPrefix.c_str(), msg), idVec4( 0.0f, 0.81f, 0.94f, 1.0f ), true, declManager->FindMaterial( "textures/bigchars" ) );
...
void idCommonLocal::Init( int argc, const char **argv, const char *cmdline ) {
	try {
		...
		if(cmdline != NULL)
		{
			if(!strstr(cmdline, "editor")) {
				devToolsStartup = false;
			}
			else {
				devToolsStartup = true;
			}	
		}
