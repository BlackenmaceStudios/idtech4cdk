...
{
	rvGENavigator* nav = (rvGENavigator*) GetWindowLongPtr ( hWnd, GWL_USERDATA );
	...
	switch ( msg )
	{
		...
		case WM_CREATE:
		{
			...
			SetWindowLongPtr ( hWnd, GWL_USERDATA, (LONG_PTR)nav );
			...
			nav->mListWndProc = (WNDPROC)GetWindowLongPtr ( nav->mTree, GWL_WNDPROC );
			SetWindowLongPtr ( nav->mTree, GWL_USERDATA, (LONG_PTR)nav );
			SetWindowLongPtr ( nav->mTree, GWL_WNDPROC, (LONG_PTR)ListWndProc );
