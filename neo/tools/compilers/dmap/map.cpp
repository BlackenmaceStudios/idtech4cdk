...
const char *VT_GetNextMaterial() {
	static int val = 0;
	return  va( "%s_%d", "default_material", val++ );
}
static void ParseBrush( const idMapBrush *mapBrush, int primitiveNum ) {
	...
	for ( i = 0 ; i < mapBrush->GetNumSides() ; i++ ) {
		...
		{
			((idMapBrushSide *)ms)->SetMaterial(VT_GetNextMaterial());
		}
...
static void CreateMapLight( const idMapEntity *mapEnt ) {
	...
	renderSystem->DeriveLightData( &light->def );
...
void FreeDMapFile( void ) {
	...
	for ( i = 0; i < dmapGlobals.mapLights.Num(); i++ ) {
		renderSystem->FreeLightDefDerivedData( &dmapGlobals.mapLights[i]->def );
	}
