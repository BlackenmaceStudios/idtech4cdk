/**
 * $Id: mallocn.c,v 1.9 2005/11/17 14:48:11 hos Exp $
...
int MEM_check_memory_integrity()
{
	...
	listend = (MemHead*)membase->last;									// (MemHead*) Added by Swanny 19/05/05
...
{
	if (vmemh) {
		MemHead *memh= (MemHead*)vmemh;									// (MemHead*) Added by Swanny 19/05/05
...
static void addtail(localListBase *listbase, void *vlink)
{
	struct localLink *link= (localLink*)vlink;									// (localLink*) Added by Swanny 19/05/05

	if (link == 0) return;
	if (listbase == 0) return;

	link->next = 0;
	link->prev = (localLink*)listbase->last;									// (localLink*) Added by Swanny 19/05/05
...
static char *check_memlist(MemHead *memh)
{
	...
	forw = (MemHead*)membase->first;								// (MemHead*) Added by Swanny 19/05/05
