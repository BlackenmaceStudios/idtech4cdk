		       ...
		       Stype_t stype, Dtype_t dtype, Mtype_t mtype)
{
    ...
    Astore = (NCformat*)A->Store;										// (NCformat) added by Swanny 19/05/05
    Astore->nnz = nnz;
    Astore->nzval = nzval;
		       ...
		       Stype_t stype, Dtype_t dtype, Mtype_t mtype)
{
    ...
    if ( !(A->Store) ) ABORT("SUPERLU_MALLOC fails for A->Store");
    Astore = (NRformat*)A->Store;										// (NRformat*) added by Swanny 19/05/05
    Astore->nnz = nnz;
    Astore->nzval = nzval;
			...
			Stype_t stype, Dtype_t dtype, Mtype_t mtype)
{
    ...
    Lstore = (SCformat*)L->Store;										// (SCformat*) added by Swanny 19/05/05
    Lstore->nnz = nnz;
         ...
         SuperMatrix *A, SuperMatrix *B)
{
    ...
    Astore = (NCformat*)A->Store;				// (NCformat*)
    Aval   = (float *) Astore->nzval;
    Bstore = (DNformat*)B->Store;				// (DNFormat*) added by Swanny 19/05/05
    rhs    = (float*)Bstore->nzval;				// (float*)
    ldc    = Bstore->lda;
...
void sinf_norm_error(int nrhs, SuperMatrix *X, float *xtrue)
{
    ...
    Xstore = (DNformat*)X->Store;				// (DNformat*) added by Swanny 19/05/05
    Xmat = (float*)Xstore->nzval;				// (float*)

    for (j = 0; j < nrhs; j++) {
      soln_work = &Xmat[j*Xstore->lda];
      err = xnorm = 0.0;
      for (i = 0; i < X->nrow; i++) {
	err = (float)SUPERLU_MAX(err, fabs(soln_work[i] - xtrue[i]));
	xnorm = (float)SUPERLU_MAX(xnorm, fabs(soln_work[i]));
      }
      err = err / xnorm;
      printf("||X - Xtrue||/||X|| = %e\n", err);
    }
