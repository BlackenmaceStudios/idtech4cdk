...
int sQuerySpace(SuperMatrix *L, SuperMatrix *U, mem_usage_t *mem_usage)
{
    ...
    Lstore = (SCformat*)L->Store;				// (SCformat*)
    Ustore = (NCformat*)U->Store;				// (NCformat*)
    n = L->ncol;
    iword = sizeof(int);
    dword = sizeof(float);
	  ...
	  int **iwork, float **dwork)
{
	...
	nzlmax = (int)SUPERLU_MAX(1, FILL/4.) * annz;
...
void copy_mem_float(int howmany, void *old, void *newa)
{
    register int i;
    float *dold = (float*)old;					// (float*)
    float *dnew = (float*)newa;					// (float*)
...
/*		    new_mem = (void *) calloc(new_len, lword); */
	...
	if ( no_expand == 0 ) {
		...
		if ( type < USUB ) 
		{
		    //Glu->usub = expanders[USUB].mem = (void*)((char*)expanders[USUB].mem + extra);
			expanders[USUB].mem = (void*)((char*)expanders[USUB].mem + extra);
			Glu->usub = (int*)((char*)expanders[USUB].mem + extra);
		}
		if ( type < LSUB ) 
		{
		    //Glu->lsub = expanders[LSUB].mem = (void*)((char*)expanders[LSUB].mem + extra);
		    expanders[LSUB].mem = (void*)((char*)expanders[LSUB].mem + extra);
			Glu->lsub = (int*)((char*)expanders[LSUB].mem + extra);
		}
		if ( type < UCOL ) 
		{
		    //Glu->ucol = (float*)expanders[UCOL].mem = (void*)((char*)expanders[UCOL].mem + extra);		// (float*) Swanny
			expanders[UCOL].mem = (void*)((char*)expanders[UCOL].mem + extra);		// (float*) Swanny
			Glu->ucol = (float*)((char*)expanders[UCOL].mem + extra);
		}
		stack.top1 += extra;
		stack.used += extra;
		if ( type == UCOL ) {
