 ...
 */
    ...
    Astore   = (NCPformat*)A->Store;			// (NCPformat*)
    a        = (float*)Astore->nzval;			// (float*)
    asub     = Astore->rowind;
    ...
    *info = sLUMemInit(fact, work, lwork, m, n, Astore->nnz,
	...
	if ( relax_end[jcol] != EMPTY ) { /* start of a relaxed snode */
		...
		if ( (*info = spivotL(icol, (float)diag_pivot_thresh, &usepr, perm_r, iperm_r, iperm_c, &pivrow, &Glu, stat)) )
		...
		if ((*info = scopy_to_ucol(jj, nseg, segrep, &repfnz[k],
	    	...
	    	if ( (*info = spivotL(jj, (float)diag_pivot_thresh, &usepr, perm_r, iperm_r, iperm_c, &pivrow, &Glu, stat)) )
		    if ( iinfo == 0 ) iinfo = *info;
