...
int rvRollupPanel::InsertItem ( const char* caption, HWND dialog, bool autoDestroy, int index )
{
	...
	item->mOldDlgProc	 = (WNDPROC) GetWindowLongPtr ( dialog, DWL_DLGPROC );
	item->mOldButtonProc = (WNDPROC) GetWindowLongPtr ( button, GWL_WNDPROC );
	...
	SetWindowLongPtr ( dialog, GWL_USERDATA,	(LONG_PTR)item );

	// Attach item to button through user data
	SetWindowLongPtr ( button, GWL_USERDATA,	(LONG_PTR)item );
	SetWindowLongPtr ( button, GWL_ID,			index );

	// Subclass dialog
	SetWindowLongPtr ( dialog, DWL_DLGPROC, (LONG_PTR)DialogProc );

	// SubClass button
	SetWindowLongPtr ( button, GWL_WNDPROC, (LONG_PTR)ButtonProc );
...
{
	RPITEM*			item  = (RPITEM*)GetWindowLongPtr ( hWnd, GWL_USERDATA );
	rvRollupPanel*	_this = (rvRollupPanel*)GetWindowLongPtr ( GetParent ( hWnd ), GWL_USERDATA );
...
{
	rvRollupPanel* panel;
	panel = (rvRollupPanel*)GetWindowLongPtr (hWnd, GWL_USERDATA);	
	...
	switch ( uMsg )
	{
		case WM_CREATE:
		{
			...
			SetWindowLongPtr ( hWnd, GWL_USERDATA, (LONG_PTR)panel );
			break;
		}
