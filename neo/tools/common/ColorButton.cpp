...
void ColorButton_SetColor ( HWND hWnd, COLORREF color )
{
	...
	SetWindowLongPtr ( hWnd, GWL_USERDATA, color );
...
{
	return (COLORREF) GetWindowLongPtr ( hWnd, GWL_USERDATA );
}
...
void ColorButton_DrawItem ( HWND hWnd, LPDRAWITEMSTRUCT dis )
{
	...
	{
		HBRUSH color = CreateSolidBrush ( (COLORREF)GetWindowLongPtr ( hWnd, GWL_USERDATA ) );
