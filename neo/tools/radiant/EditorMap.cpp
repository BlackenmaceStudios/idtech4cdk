...
idRenderModel *oldSelectedModelEntity = NULL;
...
void AddEntityToManaged( idMapEntity *mapent )
{
	idStr classname = mapent->epairs.GetString("classname");
	idStr name = mapent->epairs.GetString("name");

	toolsManaged->AddEntity(_bstr_t(classname.c_str()), _bstr_t(name.c_str()));
}
...
// jmarshall
	// Init the default mega project.
	g_qeglobals.megaproject.InitDefault();

	// Tell the managed code were opening a new map.
	toolsManaged->NewMapEvent();
// jmarshall end
}
