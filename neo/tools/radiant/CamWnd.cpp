...
void CCamWnd::OnDestroy() {
	idManagedEditorWindow::OnDestroy();
}
...
int CCamWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) {
	if (idManagedEditorWindow::OnCreate(lpCreateStruct) == -1) {
		return -1;
	}
...
void CCamWnd::OriginalMouseDown(UINT nFlags, CPoint point) {
	...
	Cam_MouseDown(point.x, (r.bottom - r.top) - point.y, nFlags);
}
...
// jmarshall
	if( (buttons == MK_RBUTTON) && Select_AreObjectsSelected()) 
	{
		...
		inEntityProperties = false;
		selectedEntity = NULL;
		oldSelectedModelEntity = NULL;
	...
	OpenEntityContextMenu(NULL); // Disable the entity context menu.
...
void CCamWnd::Cam_Draw() {
	...
	g_qeglobals.sw->PlaceListener( m_Camera.origin, axis, 0, sys->Milliseconds(), "Undefined" );
...
void CCamWnd::OnSize(UINT nType, int cx, int cy) {
	idManagedEditorWindow::OnSize(nType, cx, cy);
	...
	if (brush->modelHandle > 0 ) {
		...
		if (model) {
			...
			for (int i = 0; i < model->NumSurfaces() ; i++) {
				...
				srfTriangles_t *tri2 = renderTriSurfaceManager->CopyStaticTriSurf(tri);
...
int Brush_ToTris(brush_t *brush, idTriList *tris, idMatList *mats, bool models, bool bmodel) {
	...
	if ( brush->pPatch ) {
		...
		tri = renderTriSurfaceManager->AllocStaticTriSurf();
		tri->numVerts = width * height;
		tri->numIndexes = 6 * ( width - 1 ) * ( height - 1 );
		renderTriSurfaceManager->AllocStaticTriSurfVerts( tri, tri->numVerts );
		renderTriSurfaceManager->AllocStaticTriSurfIndexes( tri, tri->numIndexes );
...
void Select_ToOBJ() {
	...
	if (dlgFile.DoModal() == IDOK) {
		...
		for( i = 0; i < tris.Num(); i++ ) {
			renderTriSurfaceManager->FreeStaticTriSurf( tris[i] );
		}
		tris.Clear();
	}
}
...
void Select_ToCM() {
	CFileDialog dlgFile( FALSE, "fbx, ase", NULL, 0, "(*.fbx)|*.fbx|(*.ase)|*.ase|(*.ma)|*.ma||", g_pParentWnd );
...
// jmarshall
//	renderSystem->CropRenderSize(m_Camera.width, m_Camera.height);
