...
/*
====================
idImage::ReadDriverPixels
====================
*/
byte *idImage::ReadDriverPixels( int x, int y, int width, int height) {
	if(width <= 0) {
		width = uploadWidth;
	}
	...
	qglGetTexImage( GL_TEXTURE_2D, 0, GL_RGBA, GL_UNSIGNED_BYTE, rawBuffer );
	GL_CheckErrors();
		...
		for (z = 0; z < Depth; z++) {
			for (y = 0; y < Height; y++) {
				for (x = 0; x < Width; x++) {
					...
					Dest[DestIndex + 0] = (byte )color.x;
					Dest[DestIndex + 1] = (byte )color.y;
					Dest[DestIndex + 2] = (byte )color.z;
					Dest[DestIndex + 3] =(byte )color.w;
				}
			}
		}
