...
// bmRenderShadowMap
//
class bmRenderShadowMap {
public:
			bmRenderShadowMap();
	void	Clear( void );
	void	Init( const char *lightName, idRenderLightLocal *light );
	
	void	RenderShadowMap( drawSurf_t	 *drawSurfs, int side  );
	bool    IsValid() { return depthCubeMap != NULL; }
	idImage *depthCubeMap;

	void    RenderLightView( idRenderLightLocal *light );
...
===============
*/
ID_INLINE bmRenderShadowMap::bmRenderShadowMap() {
	Clear();
}

ID_INLINE void bmRenderShadowMap::Clear( void ) {
	depthCubeMap = NULL;
}
...
public:
	...
	virtual renderEntity_t *GetParms() { return &parms; }
...
// jmarshall
	int					vLightNum;
	bmShadowMapCache    shadowMapCache;
// jmarshall end
...
// jmarshall
	const idMaterial *		postDiffuseMtr;

	bmRenderProgramConstantBuffer *cbRenderScene;
// jmarshall end
...
// cvars
//
extern idCVar r_backend_shadowMapCacheSize;
...
void	GL_CheckErrors( bool error = false );
...
bool R_CheckExtension( char *name, bool required = false );
...
bmRenderProgram	*R_FindARBProgram( const char *program, bool force = false );
...
typedef struct {
	char			name[64];
	int				numPasses;
	bmRenderProgram *programHandle;
} progDef_t;

extern bmRenderProgram *activeRenderProgram;

typedef enum {
	VTPASS_REMAPINDEXES = 0,
	VTPASS_UNPACKTILES
} virtualTexturePass_t;

typedef enum {
	PROG_PREINTERACTION = 0,
	PROG_INTERACTION,
	PROG_SHADOW,
	PROG_DEPTH,
	PROG_UNLITTRANSPARANCY,
	PROG_VIRTUALTEXTURE,
	PROG_DIFFUSEONLY,
	PROG_LAYERS
} progHandle_t;
...
void RB_Deferred_ShadowPass( viewLight_t *lights, drawSurf_t **drawSurfs, int numDrawSurfs );
/*
=============================================================
...
void RB_VirtualTexture_Prepass( drawSurf_t	 **drawSurfs, int numDrawSurfs, int areaID );
