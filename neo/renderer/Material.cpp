...
idImage* idMaterial::GetDiffuseImage( void )  const {
	for ( int surfaceStageNum = 0 ; surfaceStageNum < GetNumStages() ; surfaceStageNum++ ) {
		...
		switch( surfaceStage->lighting ) {
			case SL_DIFFUSE: {
				return surfaceStage->texture.image;
			}
		}
	}
...
void idMaterial::ParseMaterial( idLexer &src ) {
	...
	while ( 1 ) {
		...
		else if (!token.Icmp( "DeferredInteraction")) {
			isDeferredInteraction = true;
			continue;
		}
		else if ( token == "{" ) {
...
bool idMaterial::Parse( const char *text, const int textLength ) {
	...
	if ( coverage == MC_BAD ) {
		...
		if ( !numStages ) {
			// non-visible
			coverage = MC_OPAQUE; // jmarshall - MC_OPAQUE
...
/*
===================
idMaterial::IsDeferredInteraction
===================
*/
bool idMaterial::IsDeferredInteraction()  const { 
	return isDeferredInteraction; 
}
/*
===================
