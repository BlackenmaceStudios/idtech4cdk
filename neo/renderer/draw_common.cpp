...
=====================
*/
int RB_STD_DrawShaderPasses( drawSurf_t **drawSurfs, int numDrawSurfs, bool inGameSurfaces = false, bool onlyDeferredInteraction = false ) {
	int				i;
	...
	//RB_SetProgramEnvironment(); // jmarshall 
	...
	for (i = 0  ; i < numDrawSurfs ; i++ ) {
		...
		if ( drawSurfs[i]->material->GetSort() >= SS_POST_PROCESS  && !drawSurfs[i]->material->HasGui()
			&& !backEnd.currentRenderCopied && inGameSurfaces ) {
			break;
		}

		if(onlyDeferredInteraction) {
			if(r_deferredRenderer.GetBool() && drawSurfs[i]->material->IsDeferredInteraction()) {
				RB_Deferred_DrawDeferredInteraction( drawSurfs[i] );
				break;
			}
			continue; // Don't draw the deferred interaction if we aren't using the deferred renderer.
		}
		else if( drawSurfs[i]->material->IsDeferredInteraction() && !r_deferredRenderer.GetBool() ) {
			continue;
		}
		else if(drawSurfs[i]->material->Deform() != DFRM_NONE || drawSurfs[i]->material->Texgen() == TG_WOBBLESKY_CUBE) {
			if(!inGameSurfaces ) {
				continue;
			}
		}
		else {
			if(inGameSurfaces && !drawSurfs[i]->material->HasGui()) {
				continue;
			}
		}

		RB_STD_T_RenderShaderPasses( drawSurfs[i], false );
	}
...
void RB_STD_CreateShadowMaps( void ) {
	viewLight_t *vLight = NULL;

	// Clear the screen map cache. 
	backEnd.shadowMapCache.ClearScreenMapCache();
	...
	for ( vLight = backEnd.viewDef->viewLights ; vLight ; vLight = vLight->next ) {
		...
		if(vLight->lightDef->parms.lightRadius.x <= 0 && vLight->lightDef->parms.lightRadius.y <= 0 && vLight->lightDef->parms.lightRadius.z <= 0) {
			continue;
		}
		...
		vLight->lightDef->shadowMap.RenderLightView( vLight->lightDef );
		
		backEnd.shadowMapCache.RenderToScreenBuffer( vLight );
...
=====================
*/
void RB_BeginVirtualTexturePass( void ) {
...
=====================
*/
void RB_EndVirtualTexturePass( void ) {
	
}

/*
=====================
RB_RenderVirtualTextureArea
=====================
*/
void RB_RenderVirtualTextureArea( int areaNum ) {
	...
	// Render the surfaces out to the apprioiate render targets.
	RB_VirtualTexture_Prepass( drawSurfs, numDrawSurfs, areaNum );
}
