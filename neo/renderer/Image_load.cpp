...
GLenum idImage::SelectInternalFormat( const byte **dataPtrs, int numDataPtrs, int width, int height,
									 textureDepth_t minimumDepth, bool *monochromeResult, imageCompression_t compress ) const {
	int		i, c;
	const byte	*scan;
	...
	*monochromeResult = true;	// until shown otherwise
	...
	if(compress != IMAGE_COMPRESS_NONE) {
		switch(compress) {
			case IMAGE_COMPRESS_DXT1:
				return GL_COMPRESSED_RGBA_S3TC_DXT1_EXT;

			case IMAGE_COMPRESS_DXT5:
...
================
*/
void idImage::Generate2DImageAtlas( int atlasSize, int tileSize, bool intStorage ) {
	...
	memset(buffer, 0, atlasSize * atlasSize * 4);

	idVec4 tile = idVec4(0, 0, 0, 0);


	for(int h = 0; h < atlasSize / tileSize; h++) {
		for(int w = 0; w < atlasSize / tileSize; w++) {
			tile.z = w;
			tile.w = h;
			...
			//}
				...
				common->FatalError("Too little data in color");
				return;
			}
		}
	}
...
================
*/
void idImage::GenerateRectImage( const byte *pic, int width, int height ) {
	PurgeImage();
...
void idImage::GenerateImage( const byte *pic, int width, int height, 
	...
	// jmarshall - don't care about power of two limitations.
	scaled_width = width;
	...
	internalFormat = SelectInternalFormat( &pic, 1, width, height, depth, &isMonochrome, compress );
	...
	if ( ( scaled_width == width ) && ( scaled_height == height ) ) {
		...
		if(pic == NULL)
		{
			memset(scaledBuffer, 0, sizeof( unsigned ) * scaled_width * scaled_height );
		}
		else
		{
...
*/
	...
	else if(compress != IMAGE_COMPRESS_NONE)
	{
		// This won't work for DXT1!!!
		qglCompressedTexImage2DARB( GL_TEXTURE_2D, 0, internalFormat, scaled_width, scaled_height, 0, width * height, scaledBuffer );
	} 
	else {
	...
	while ( scaled_width > 1 || scaled_height > 1 ) {
		...
		{
			// This won't work for DXT1!!!
			//qglCompressedTexImage2DARB( GL_TEXTURE_2D, miplevel, internalFormat, scaled_width, scaled_height, 0, width * height, scaledBuffer );
		}
...
void idImage::Generate3DImage( const byte *pic, int width, int height, int picDepth,
	...
	internalFormat = SelectInternalFormat( &pic, 1, width, height * picDepth, minDepthParm, &isMonochrome, IMAGE_COMPRESS_NONE );
...
// jmarshall end
	...
	if ( cubeFiles != CF_2D ) {
		...
		if ( pic == NULL ) {
			common->Warning( "Couldn't load image: %s - DEFAULTING", imgName.c_str() );
			MakeDefault();
			return;
		}
/*
...
void idImage::Bind() {
	...
	if ( type == TT_2D ) {
		...
		if ( tmu->currentRectMap != texnum ) {
			tmu->currentRectMap = texnum;
...
void idImage::UploadScratch( const byte *data, int cols, int rows ) {
	...
	if ( rows == cols * 6 ) {
		...
		// if the scratchImage isn't in the format we want, specify it as a new texture
		if ( (cols != uploadWidth || rows != uploadHeight) || type == TT_RECT ) {
			uploadWidth = cols;
			uploadHeight = rows;
			qglTexImage2D( target, 0, GL_RGB8, cols, rows, 0, GL_RGBA, GL_UNSIGNED_BYTE, data );
		} else {
			// otherwise, just subimage upload it so that drivers can tell we are going to be changing
			// it and don't try and do a texture compression
