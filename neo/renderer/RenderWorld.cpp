...
===================
*/
idRenderEntity	*idRenderWorldLocal::AddClientEntity( idDict &spawnArgs ) {
	renderEntity_t re;
	bool shouldFree = false;
	const char *modelpath = spawnArgs.GetString( "model" );

	memset(&re, 0, sizeof(renderEntity_t));

	// Check to see if this
	if(modelpath != NULL && strlen(modelpath) > 0) {
		re.hModel = renderModelManager->FindModel( modelpath );
	}
	else {
		re.hModel = renderModelManager->DefaultModel();
		shouldFree = true;
	}

	re.origin = spawnArgs.GetVector( "origin" );
	re.axis = spawnArgs.GetMatrix( "rotation" );

	qhandle_t handle = AddEntityDef( &re );
	idRenderEntityLocal *ent = entityDefs[handle];

	ent->parms.clientEntity = game->SpawnClientEntity( spawnArgs, (idRenderEntity *)ent, this );
	if(shouldFree || ent->parms.clientEntity == NULL) {
		FreeEntityDef( handle );
		return NULL;
	}
...
===================
*/
void idRenderWorldLocal::FreeClientEntity( idRenderEntity *entity ) {
	idRenderEntityLocal *ent = (idRenderEntityLocal *)entity;

	if(ent->parms.clientEntity == NULL) {
		common->Warning("FreeClientEntity: Tried to free NULL clientEntity\n");
		return;
	}
}
// jmarshall end
...
// jmarshall
	if(def->parms.clientEntity != NULL) {
		FreeClientEntity( def );
	}
// jmarshall end
...
==================
*/
idRenderLight   *idRenderWorldLocal::GetRenderLightInterface( qhandle_t lightHandle ) const {
	idRenderLightLocal *def;
...
// jmarshall
	for(int i = 0; i < entityDefs.Num(); i++) {
		...
		game->UpdateClientEntity( def->parms.clientEntity );
	}
// jmarshall end
