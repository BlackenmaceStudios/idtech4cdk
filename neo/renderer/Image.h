...
// jmarshall
	virtual void		GenerateRectImage( const byte *pic, int width, int height );
	virtual void		Generate2DImageAtlas( int atlasSize, int tileSize, bool intStorage );
// jamrshall end
...
/*
=================
R_CopyImageToImageBufferRegion
=================
*/
template<typename T>
ID_INLINE void R_CopyImageToImageBufferRegion( T *Dest, T *color,  int DestX, int DestY, unsigned int Width, unsigned int Height, int DiemWidth ) {
		...
		for (z = 0; z < Depth; z++) {
			for (y = 0; y < Height; y++) {
				for (x = 0; x < Width; x++) {
					...
					Dest[DestIndex + 0] = (T)color[SrcIndex + 0];
					Dest[DestIndex + 1] = (T)color[SrcIndex + 1];
					Dest[DestIndex + 2] = (T)color[SrcIndex + 2];
					Dest[DestIndex + 3] =(T)color[SrcIndex + 3];
				}
			}
		}
