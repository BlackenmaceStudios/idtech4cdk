...
private:
	...
	bool						ScaleUVRegionToFitInTri( bmVTModel *model, srfTriangles_t *parentTris, srfTriangles_t *tris, int triId, int pageId, int widthId, int heightId, float uvScaleW, float uvScaleH, float chartW, float chartH );
	int							RemapVertexFromParentToChildTri( srfTriangles_t *parentTris, idList<idDrawVert> &childVerts,  int index, bool appendVert, bool allowNewNeighbor = false );

	idList< idDrawVert >		verts;
	int	numCalcedAreas;
	idList<idDrawVert> storedTris;
};
