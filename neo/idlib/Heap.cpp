...
// jmarshall
bmMemoryHandler		*allocator = NULL;
// jmarshall end

#define MEMORY_LARGE_INT	__int64
...
public:
	...
	void *			Allocate( const MEMORY_LARGE_INT bytes );	// allocate memory
	void			Free( void *p );				// free memory
	void *			Allocate16( const MEMORY_LARGE_INT bytes );// allocate 16 byte aligned memory
	void			Free16( void *p );				// free 16 byte aligned memory
	MEMORY_LARGE_INT			Msize( void *p );				// return size of data block
	void			Dump( void  );
...
private:
	...
	struct page_s {									// allocation page
		...
		MEMORY_LARGE_INT				dataSize;				// number of bytes of memory 'data' points to
		page_s *			next;					// next free page in same page manager
		page_s *			prev;					// used only when allocated
		MEMORY_LARGE_INT				largestFree;			// this data used by the medium-size heap manager
		void *				firstFree;				// pointer to first free entry
	};

	struct mediumHeapEntry_s {
		page_s *			page;					// pointer to page
		MEMORY_LARGE_INT				size;					// size of block
		...
		MEMORY_LARGE_INT				freeBlock;				// non-zero if free block
	};
	...
	MEMORY_LARGE_INT			smallCurPageOffset;				// byte offset in current page
	...
	MEMORY_LARGE_INT			pagesAllocated;					// number of pages currently allocated
	MEMORY_LARGE_INT			pageSize;						// size of one alloc page in bytes

	MEMORY_LARGE_INT			pageRequests;					// page requests
	MEMORY_LARGE_INT			OSAllocs;						// number of allocs made to the OS
	...
	// methods
	page_s *		AllocatePage( MEMORY_LARGE_INT bytes );	// allocate page from the OS
	void			FreePage( idHeap::page_s *p );	// free an OS allocated page

	void *			SmallAllocate( MEMORY_LARGE_INT bytes );	// allocate memory (1-255 bytes) from small heap manager
	void			SmallFree( void *ptr );			// free memory allocated by small heap manager

	void *			MediumAllocateFromPage( idHeap::page_s *p, MEMORY_LARGE_INT sizeNeeded );
	void *			MediumAllocate( MEMORY_LARGE_INT bytes );	// allocate memory (256-32768 bytes) from medium heap manager
	void			MediumFree( void *ptr );		// free memory allocated by medium heap manager

	void *			LargeAllocate( MEMORY_LARGE_INT bytes );	// allocate large block from OS directly
...
================
*/
void *idHeap::Allocate( const MEMORY_LARGE_INT bytes ) {
	if ( !bytes ) {
		return NULL;
	}
...
================
*/
void *idHeap::Allocate16( const MEMORY_LARGE_INT bytes ) {
	byte *ptr, *alignedPtr;
...
================
*/
MEMORY_LARGE_INT idHeap::Msize( void *p ) {
	...
	switch( ((byte *)(p))[-1] ) {
		...
		case LARGE_ALLOC: {
			return ((idHeap::page_s*)(*((MEMORY_LARGE_INT *)(((byte *)p) - ALIGN_SIZE( LARGE_HEADER_SIZE )))))->dataSize - ALIGN_SIZE( LARGE_HEADER_SIZE );
		}
		default: {
...
================
*/
idHeap::page_s* idHeap::AllocatePage( MEMORY_LARGE_INT bytes ) {
	idHeap::page_s*	p;
...
================
*/
void *idHeap::SmallAllocate( MEMORY_LARGE_INT bytes ) {
	// we need the at least sizeof( MEMORY_LARGE_INT ) bytes for the free list
	if ( bytes < sizeof( MEMORY_LARGE_INT ) ) {
		bytes = sizeof( MEMORY_LARGE_INT );
	}
	...
	if ( smallBlock ) {
		MEMORY_LARGE_INT *link = (MEMORY_LARGE_INT *)(smallBlock + SMALL_HEADER_SIZE);
		smallBlock[1] = SMALL_ALLOC;					// allocation identifier
		smallFirstFree[bytes / ALIGN] = (void *)(*link);
		return (void *)(link);
	}

	MEMORY_LARGE_INT bytesLeft = (long)(pageSize) - smallCurPageOffset;
...
void idHeap::SmallFree( void *ptr ) {
	...
	MEMORY_LARGE_INT *dt = (MEMORY_LARGE_INT *)ptr;
	// index into the table with free small memory blocks
	MEMORY_LARGE_INT ix = *d;
	...
	*dt = (MEMORY_LARGE_INT)smallFirstFree[ix];	// write next index
...
================
*/
void *idHeap::MediumAllocateFromPage( idHeap::page_s *p, MEMORY_LARGE_INT sizeNeeded ) {
	...
	if ( best->size >= (MEMORY_LARGE_INT)( sizeNeeded + MEDIUM_SMALLEST_SIZE ) ) {
...
================
*/
void *idHeap::MediumAllocate( MEMORY_LARGE_INT bytes ) {
	idHeap::page_s		*p;
	void				*data;

	MEMORY_LARGE_INT sizeNeeded = ALIGN_SIZE( bytes ) + ALIGN_SIZE( MEDIUM_HEADER_SIZE );
...
================
*/
void *idHeap::LargeAllocate( MEMORY_LARGE_INT bytes ) {
	...
	MEMORY_LARGE_INT *	dw	= (MEMORY_LARGE_INT*)(d - ALIGN_SIZE( LARGE_HEADER_SIZE ));
	dw[0]		= (MEMORY_LARGE_INT)p;				// write pointer back to page table
...
void idHeap::LargeFree( void *ptr) {
	...
	// get page pointer
	pg = (idHeap::page_s *)(*((MEMORY_LARGE_INT *)(((byte *)ptr) - ALIGN_SIZE( LARGE_HEADER_SIZE ))));
...
void *Mem_Alloc16( const int size ) {
	return allocator->AllocAlign( size, 16 );
}
