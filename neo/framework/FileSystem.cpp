...
private:
	...
	static idFile_Memory	tempMemoryFile;
...
// jmarshall
	static idCVar			fs_generatedFolder;
// jmarshall end
...
idFile_Memory	idFileSystemLocal::tempMemoryFile;
...
// jmarshall
idCVar	idFileSystemLocal::fs_generatedFolder( "fs_generatedFolder", "generated", CVAR_SYSTEM | CVAR_ROM, "folder that contains all the generated game data." );
// jmarshall end
...
// jmarshall - fix fs_restrict workaround.
#ifdef ID_DEMO_BUILD
	common->StartupVariable( "fs_restrict", false );
#else
	common->StartupVariable( "fs_restrict", true );
#endif
// jmarshall end
...
// jmarshall
		fs_savepath.SetString( va("%s/%s", Sys_DefaultSavePath(), GAME_NAME) );
		common->Printf("Current User SavePath: \n");
		common->Printf("...%s\n", fs_savepath.GetString());
// jmarshall end
	}
	else
	{
		common->Warning("Using custom savepath, the application may or may not have access to write to this folder!\n");
	}
...
// jmarshall
	idStr generatedFolder = fs_generatedFolder.GetString();
	
	if (!fs_restrict.GetBool())
	{

		// Set fs_devpath first cause the generated folder is the default value for that cvar.
		if (fs_devpath.GetString()[0] == '\0')
		{
			fs_devpath.SetString( va("%s/%s/%s", fs_cdpath.GetString()[0] ? fs_cdpath.GetString() : fs_basepath.GetString(), gameFolder.c_str(), generatedFolder.c_str() ) );
		}
		else
		{
			fs_devpath.SetString( va("%s/%s", fs_devpath.GetString(), fs_generatedFolder.GetString() ) );
		}

		common->Printf("Developer/Generated Path: \n");
		common->Printf("...%s\n", fs_devpath.GetString() );
	}
	else
	{
		fs_generatedFolder.SetString("");
...
// jmarshall 
	...
	if(!fs_restrict.GetBool())
	{
		f = OpenFileReadFlags( relativePath, FSFLAG_SEARCH_DIRS | FSFLAG_SEARCH_PAKS, NULL, allowCopyFiles, gamedir );
	...
	// Try to open the file from the generated folder.
	return OpenFileReadFlags( va("%s/%s",fs_generatedFolder.GetString(), relativePath), FSFLAG_SEARCH_DIRS | FSFLAG_SEARCH_PAKS, NULL, allowCopyFiles, gamedir );
// jmarshall end
}
