...
void idPlayer::ClearFocus( void ) {
	...
	focusGUIent		= NULL;
	focusUI			= NULL;
	vehicleController.ClearFocusedVehicle();
	talkCursor		= 0;
}
...
void idPlayer::UpdateFocus( void ) {
	...
	for ( i = 0; i < listedClipModels; i++ ) {
		...
		if ( allowFocus ) {
			...
			if ( ent->IsType( bmVehicle_Base::Type ) ) {
				if(vehicleController.UpdateVehicleFocus( ent, start, end )) {
					focusTime = gameLocal.time + FOCUS_TIME;
					break;
				}
				continue;
...
================
*/
bool idPlayer::IsInGame() {
	return playerView.IsValid();
}
...
void idPlayer::UpdateViewAngles( void ) {
	...
	// If we are driving we need to update the view angles accordingly.
	if(vehicleController.IsDriving()) {
		idAngles angle = viewAngles;
		vehicleController.UpdateViewAngles( angle );
	
		for ( i = 0; i < 3; i++ ) {
			usercmd.angles[i] += angle[i];
		}
	}
...
void idPlayer::SetClipModel( void ) {
	...
	if ( spectating ) {
		...
		bounds[0].Set( -pm_bboxwidth.GetFloat() * 0.8f, -pm_bboxwidth.GetFloat() * 0.8f, 0 );
		bounds[1].Set( pm_bboxwidth.GetFloat() * 0.8f, pm_bboxwidth.GetFloat() * 0.8f, pm_normalheight.GetFloat() );
	}
...
void idPlayer::UseVehicle( void ) {
	...
	if ( GetBindMaster() && GetBindMaster()->IsType( bmVehicle_Base::Type ) ) {
		Show();
		static_cast<bmVehicle_Base*>(GetBindMaster())->Use( this );
	} else {
		start = GetEyePosition();
		end = start + viewAngles.ToForward() * 80.0f;
		gameLocal.clip.TracePoint( trace, start, end, MASK_SHOT_RENDERMODEL, this );
		if ( trace.fraction < 1.0f ) {
			...
			if ( ent && ent->IsType( bmVehicle_Base::Type ) ) {
				Hide();
				static_cast<bmVehicle_Base*>(ent)->Use( this );
			}
		}
	}
}
...
void idPlayer::PerformImpulse( int impulse ) {
	...
	switch( impulse ) {
		...
		case IMPULSE_40: {
			vehicleController.UseVehicleCommand();
			break;
		}
	} 

	if(vehicleController.IsDriving()) {
		vehicleController.PerformImpulse( impulse );
	}
}
...
// jmarshall end
	...
	} else if ( GetBindMaster() && GetBindMaster()->IsType( bmVehicle_Base::Type ) ) {
		newEyeOffset = 0.0f;
	} else {
