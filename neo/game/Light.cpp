...
void idLight::Spawn( void ) {
	...
	// Check to see if we are using dynamic lighting.
	spawnArgs.GetBool( "dynamic", "0", dynamicLighting );
...
void idLight::PresentLightDefChange( void ) {
	...
	if ( ( lightDefHandle != -1 ) ) {
		...
		light = gameRenderWorld->GetRenderLightInterface( lightDefHandle );

		// Generate the shadowmap for this light.
		if(!dynamicLighting)
		{
			
		}

		if(!gameLocal.testLightMaterial)
		{
			gameLocal.testLightMaterial = declManager->FindMaterial("renderer/testshadow");
		}
	}
}
