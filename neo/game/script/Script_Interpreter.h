...
====================
*/
ID_INLINE void idInterpreter::Push( INT_PTR value ) {
	if ( localstackUsed + sizeof( INT_PTR ) > LOCALSTACK_SIZE ) {
		Error( "Push: locals stack overflow\n" );
	}
	*( INT_PTR * )&localstack[ localstackUsed ]	= value;
	localstackUsed += sizeof( INT_PTR );
}
...
ID_INLINE varEval_t idInterpreter::GetVariable( idVarDef *def ) {
	if ( def->initialized == idVarDef::stackVariable ) {
		varEval_t val;
		val.intPtr = ( INT_PTR * )&localstack[ localstackBase + def->value.stackOffset ];
		return val;
	} else {
		return def->value;
	}
