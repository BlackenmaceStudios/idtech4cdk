...
================
*/
idSound* idAnimatedEntity::SpawnSpeakerOnJoint( jointHandle_t jointHandle, const char *soundShader, bool forceLoop ) {
	idVec3 origin;
	idMat3 axis;
	idDict args;
	idSound *speaker;

	animator.GetJointTransform( jointHandle, gameLocal.time, origin, axis );

	args.Set( "s_shader", soundShader );
	args.Set( "s_waitfortrigger", "1" ); // Default is for all the spawned speakers to be silient.
	...
	speaker = static_cast<idSound *>( gameLocal.SpawnEntityType( idSound::Type, &args ) );

	speaker->BindToJoint( this, jointHandle, true );
	speaker->UpdateSound();
