...
// jmarshall - this has to be called by the editor as well.
	virtual void				DrawDefferedPass( int width, int height ) = 0;
...
// jmarshall end
	...
	virtual void				MainMenuFrame( idUserInterface *mainMenuGui, int frametime ) = 0;

	virtual void				BeginMapChange( const char *mapPath ) = 0;
	virtual void				EndMapChange( void ) = 0;
	virtual void				LoadingFrame( int frameTime, float pct ) = 0;
};
