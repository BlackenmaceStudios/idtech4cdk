...
// jmarshall
	static CHAR path[32212]; // Prevent buffer overflow.
    HRESULT result;
	
	result = SHGetFolderPath(NULL, CSIDL_PERSONAL, NULL, SHGFP_TYPE_CURRENT, path);
	return &path[0];
// jmarshall end
}
...
IMAGE_NT_HEADERS *Sys_GetNtHeaderFromBase( void *pBaseAddr ) 
{ 
...
void *Sys_GetFuncAddr( INT_PTR pBaseAddr, const char *lpszFuncName )  
{ 
 IMAGE_NT_HEADERS       *pNtHeaders; 
 ...
 pNtHeaders = Sys_GetNtHeaderFromBase((void *)pBaseAddr); 
 if(pNtHeaders == NULL) 
  return NULL; 
 ...
 {  
  ...
  if(!strcmp(((char *)(pBaseAddr + lpszNames[dwIndex])), lpszFuncName)) 
...
//			common->Printf( "Sys_PumpEvents: win32.sysMsgTime (%i) > msg.time (%i)\n", win32.sysMsgTime, msg.time );
		...
		{
			if ( toolInterface->FrameSysMsg( &msg ) ) {	
				continue;
			}
		}
#endif
...
=================
*/
void Sys_CrashTest_f( const idCmdArgs &args ) {
	byte *someCrappyMemory;

	someCrappyMemory[666] = 'f';
}
...
// Sys_ShowWindow_f
//
void Sys_ShowWindow_f( const idCmdArgs &args )
{
...
================
*/
LONG WINAPI Sys_CrashHandler(_EXCEPTION_POINTERS *crash) 
{ 
    common->Printf( "****FAILURE**** System is now crashing...\n");

	common->Printf( "Crash detected at address: %p\n", crash->ExceptionRecord->ExceptionAddress );
...
//	SetTimer( NULL, 0, 100, NULL );
	...
	cmdSystem->AddCommand( "crashtest", Sys_CrashTest_f, CMD_FL_SYSTEM, "Tests the crash code" );
	cmdSystem->AddCommand( "showGameWindow", Sys_ShowWindow_f, CMD_FL_SYSTEM, "Shows the game window if its hidden" );
